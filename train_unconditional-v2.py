...
def parse_args():
    ...
    #############################
    # Add for printed digits
    parser.add_argument(
        "--printed_digits_dir",
        type=str,
        default=None,
        help="Path to the printed digits dataset. If provided, images from this dataset can replace MNIST digits with a probability p.",
    )
    parser.add_argument(
        "--replacement_prob",
        type=float,
...
def load_printed_digits(printed_digits_dir):
    ...
    from the corresponding folders in the specified directory.
    """
    printed_digits = {}
    total_fonts = 0
    total_images = 0
    ...
    logger.info(f"Loaded printed digits dataset with {total_fonts} fonts and {total_images} images.")
...
def main(args):
    ...
    noise_scheduler = RectFlowScheduler(num_train_timesteps=args.ddpm_num_steps)
    ...
    printed_digits = None
    if args.printed_digits_dir:
        printed_digits = load_printed_digits(args.printed_digits_dir)
        
        # Extract and log font names
        font_names = [font for font in os.listdir(args.printed_digits_dir) if os.path.isdir(os.path.join(args.printed_digits_dir, font))]
        logger.info(f"Loaded printed digits dataset with the following fonts: {', '.join(font_names)}")
